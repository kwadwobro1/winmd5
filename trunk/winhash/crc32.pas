...
begin
  ...
  begin
    Result := ((Result shr 8) and $00FFFFFF) xor
      Crc32Table[(Result xor Buffer[i]) and $000000FF];
  end;
end;

function AsmUpdateCrc32(Value: integer; Buffer: pointer; Count: integer): integer;
  assembler;
asm
...
function FileSize_(const Path: string): integer;
var
  f: file of byte;
  lang: integer;
begin
...
var
  Buffer: Pointer;
  Handle, Loaded: integer;
  anzahl, lang, erg, hilf, hilf1, last: longint;
begin
  lang := Filesize_(FileName);
  Result := $FFFFFFFF;
  GetMem(Buffer, BufferSize);
  Handle := FileOpen(FileName, fmOpenRead);
  repeat
    ...
    anzahl := anzahl + Loaded;
    hilf := anzahl div 1024;
    hilf1 := lang div 1024;
    last := erg;
    erg := hilf * 100 div hilf1;
    if erg > last then
    begin
    end;
  until Loaded <> BufferSize;
